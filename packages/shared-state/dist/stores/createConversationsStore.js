...
export function createConversationsStore(deps) {
    ...
    return create()(persist((set, get) => ({
        ...
        addMessage(conversationId, message) {
            set((state) => {
                const newMessages = new Map(state.messages);
                const current = (newMessages.get(conversationId) ?? []);
                newMessages.set(conversationId, [...current, message]);
                // Update conversation's lastMessage
                const conversations = state.conversations.map((conv) => conv.id === conversationId
                    ? {
                        ...conv,
                        lastMessage: message,
        ...
        updateMessage(conversationId, message) {
            set((state) => {
                const newMessages = new Map(state.messages);
                const current = (newMessages.get(conversationId) ?? []);
                newMessages.set(conversationId, current.map((m) => (m.id === message.id ? message : m)));
                return { messages: newMessages };
            });
        ...
        deleteMessage(conversationId, messageId) {
            set((state) => {
                const newMessages = new Map(state.messages);
                const current = (newMessages.get(conversationId) ?? []);
                newMessages.set(conversationId, current.filter((m) => m.id !== messageId));
                return { messages: newMessages };
            });
            logger?.info(`Message deleted from ${conversationId}: ${messageId}`);
        },
        // Create new conversation locally
        createConversation(conversationId, conversation) {
            set((state) => ({
        ...
        async hydrate() {
            try {
                const storedConversations = await storage.getItem("conversations:list");
                ...
                if (storedMessages) {
                    ...
                    set({ messages });
                    logger?.info(`Hydrated messages for ${messages.size} conversations`);
                }
            }
            catch (error) {
...
//# sourceMappingURL=createConversationsStore.js.map