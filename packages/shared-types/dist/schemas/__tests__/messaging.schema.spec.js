...
import { MessageSchema, MessageDTOSchema, MessageAttachmentSchema, MessageAttachmentDTOSchema, MessageReactionSchema, MessageReactionDTOSchema, MessageDecorationSchema, ConversationSchema, ConversationDTOSchema, } from "../messaging.schema.js";
...
describe("messaging.schema", () => {
    ...
    describe("MessageAttachmentDTOSchema", () => {
        it("accepts a valid MessageAttachment DTO payload", () => {
            const result = MessageAttachmentDTOSchema.safeParse(validMessageAttachmentDto);
            expect(result.success).toBe(true);
            if (result.success) {
                expect(result.data.name).toBe("document.pdf");
                expectTypeOf(result.data).toEqualTypeOf();
        ...
        it("transforms MessageAttachment DTO with snake_case to camelCase mapping", () => {
            const dtoResult = MessageAttachmentDTOSchema.safeParse(validMessageAttachmentDto);
    ...
    describe("MessageReactionDTOSchema", () => {
        ...
        it("transforms MessageReaction DTO with snake_case to camelCase mapping", () => {
            const dtoResult = MessageReactionDTOSchema.safeParse(validMessageReactionDto);
            if (dtoResult.success) {
                const transformed = snakeToCamel(dtoResult.data);
    ...
    describe("ConversationDTOSchema", () => {
        ...
        it("transforms Conversation DTO with snake_case to camelCase mapping", () => {
            ...
            if (dtoResult.success) {
                ...
                expect(transformed.lastMessageAt).toBe(validConversationDto.last_message_at);
            }
        });
    });
});
//# sourceMappingURL=messaging.schema.spec.js.map