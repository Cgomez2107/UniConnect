...
/**
 * Singleton Database
 * Centraliza la conexión al pool de PostgreSQL para el servicio de profiles-catalog.
 */
export class Database {
  private static instance: Database | null = null;
  private static activeEnv: ProfilesCatalogEnv | null = null;
  ...
  public static getInstance(env?: ProfilesCatalogEnv): Database {
    if (!Database.instance) {
      if (!env) {
        throw new Error(
          "[Database:profiles-catalog] No ha sido inicializada y no se proporcionó la configuración (env).",
        );
      }
  ...
  public async close(): Promise<void> {
    try {
      await this.pool.end();
      console.log(`[Database:profiles-catalog] Pool de conexiones cerrado exitosamente.`);
    } catch (error) {
      console.error(`[Database:profiles-catalog] Error al cerrar el pool de conexiones:`, error);
    } finally {
