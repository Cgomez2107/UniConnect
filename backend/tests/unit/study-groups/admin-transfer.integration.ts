...
describe("RequestAdminTransfer (Integration)", () => {
  it("emite el evento correcto tras una transferencia exitosa", async () => {
    ...
    const mockStudyGroupRepository = createMockStudyGroupRepository();
    ...
    mockRepository.requestTransfer.mock.mockResolvedValueOnce(mockTransfer);
    mockStudyGroupRepository.loadStudyGroup.mock.mockResolvedValueOnce(mockStudyGroup);
  ...
  it("lanza error al pedir transferencia en estado prohibido (Cerrada)", async () => {
    ...
    mockStudyGroupRepository.loadStudyGroup.mock.mockResolvedValueOnce(mockClosedGroup);
  ...
  it("no persiste ni emite si la validación de estado falla", async () => {
    ...
    mockStudyGroupRepository.loadStudyGroup.mock.mockResolvedValueOnce(mockPendingGroup);
...
describe("AcceptAdminTransfer (Integration)", () => {
  it("emite el evento correcto tras aceptación exitosa", async () => {
    ...
    mockRepository.getById.mock.mockResolvedValueOnce(mockTransfer);
    mockStudyGroupRepository.loadStudyGroup.mock.mockResolvedValueOnce(mockPendingGroup);
  ...
  it("lanza NotFoundError si la transferencia no existe", async () => {
    ...
    mockRepository.getById.mock.mockResolvedValueOnce(null);
  ...
  it("lanza error al aceptar en estado prohibido (no hay transferencia pendiente)", async () => {
    ...
    mockRepository.getById.mock.mockResolvedValueOnce(mockTransfer);
    mockStudyGroupRepository.loadStudyGroup.mock.mockResolvedValueOnce(mockAbiertaGroup);
